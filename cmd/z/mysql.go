...
func (m *_mysql) Init() {
	...
	m.db, m.err = gorm.Open(mysql.New(mysql.Config{
		DSN:                       global.ZConfig.Mysql.Dsn(), // DSN data source name
	...
	if m.err != nil {
		if strings.Split(m.err.Error(), ": ")[1] == unknownDatabase {
			...
			if n, _ := fmt.Scanln(&m.input); n != 0 {
				if m.input == "1" {
					if global.ZConfig.Mysql.Username == "root" {
						m.database()
					} else {
...
func (m *_mysql) Version() {
	...
	if err := global.ZDB.Raw("SELECT VERSION() AS version;").Scan(&m.version).Error; err != nil {
...
func (m *_mysql) Character() {
	...
	color.Debug.Println("\n[Mysql] -->获取数据库编码中.......")
	if err := global.ZDB.Raw("show variables like 'character_set_database' ").Scan(&info).Error; err != nil {
...
func (m *_mysql) utf8mb4() {
	...
	if err := global.ZDB.Debug().Exec("ALTER DATABASE " + global.ZConfig.Mysql.Dbname + " CHARACTER SET `utf8mb4` COLLATE `utf8mb4_general_ci`").Error; err != nil {
		color.Debug.Print("\n[Mysql] --> 设置数据库名为:")
		color.LightGreen.Printf(" {%v} ", global.ZConfig.Mysql.Dbname)
...
func (m *_mysql) database() {
	m.old = global.ZConfig.Mysql.Dbname
	global.ZConfig.Mysql.Dbname = "mysql"
	...
	global.ZConfig.Mysql.Dbname = m.old
	color.Debug.Print("\n[Mysql] --> 正在为您创建数据库名为:")
	color.LightGreen.Printf(" {%v} ", global.ZConfig.Mysql.Dbname)
	color.Debug.Print("中.......\n")
	if m.err = global.ZDB.Exec("CREATE DATABASE IF NOT EXISTS " + global.ZConfig.Mysql.Dbname + " DEFAULT CHARACTER SET utf8mb4 DEFAULT COLLATE utf8mb4_general_ci;").Error; m.err != nil {
		color.Debug.Print("\n[Mysql] --> 创建数据库名为:")
		color.LightGreen.Printf(" {%v} ", global.ZConfig.Mysql.Dbname)
...
func (m *_mysql) zero() {
	var info DatabaseInfo
	color.Info.Println("\n[Mysql]--> 获取数据库数据中.......")
	if err := global.ZDB.Raw("show variables like 'sql_mode';").Scan(&info).Error; err != nil {
	...
	if err := global.ZDB.Exec("set global sql_mode='" + info.Value + "';").Error; err != nil {
